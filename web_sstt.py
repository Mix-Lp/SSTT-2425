...
def main():
    ...
    try:
        ...
        thing = socket.socket(family=socket.AF_INET, type=socket.SOCK_STREAM, proto=0)
        thing.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
        thing.bind((args.host, args.port))
        thing.listen()
        while True:
            conn, addr = thing.accept()
            values = recibir_mensaje(conn)
            process_cookies(values, conn)
            #enviar_mensaje(conn,values)
            logger.info('Cliente {} connected.'.format(addr))
            conn.close()
